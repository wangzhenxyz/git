...
public class NativePlayer extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		NativePlayer.mSDLThread = new Thread(new SDLMain(), "SDLThread");

		NativePlayer.mSDLThread.start();
	...
	public void onWindowFocusChanged(boolean hasFocus) {
		...
		NativePlayer.mHasFocus = hasFocus;
		if (hasFocus) {
			NativePlayer.handleResume();
		}
	}
	...
	protected void onDestroy() {
		...
		NativePlayer.mExitCalledFromJava = true;
		NativePlayer.nativeQuit();

		// Now wait for the SDL thread to quit
		if (NativePlayer.mSDLThread != null) {
			try {
				NativePlayer.mSDLThread.join();
			} catch (Exception e) {
				Log.v("SDL", "Problem stopping thread: " + e);
			}
			NativePlayer.mSDLThread = null;
	...
	public static void handlePause() {
		if (!NativePlayer.mIsPaused && NativePlayer.mIsSurfaceReady) {
			NativePlayer.mIsPaused = true;
	...
	public static void handleResume() {
		if (NativePlayer.mIsPaused && NativePlayer.mIsSurfaceReady
				&& NativePlayer.mHasFocus) {
			NativePlayer.mIsPaused = false;
	...
	protected static class SDLCommandHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			default:
				if ((context instanceof NativePlayer)
						&& !((NativePlayer) context).onUnhandledMessage(
								msg.arg1, msg.obj)) {
...
class SDLMain implements Runnable {
	...
	public void run() {
		// Runs SDL_main()
		// SDLActivity.nativeInit("/mnt/sdcard/4008005005.wav");
...
class SDLSurface extends SurfaceView implements SurfaceHolder.Callback,
	...
	public void surfaceDestroyed(SurfaceHolder holder) {
		...
		NativePlayer.handlePause();
		NativePlayer.mIsSurfaceReady = false;
		NativePlayer.onNativeSurfaceDestroyed();
	}
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int width,
		...
		mWidth = width;
		mHeight = height;
		NativePlayer.onNativeResize(width, height, sdlFormat);
		Log.v("SDL", "Window size:" + width + "x" + height);

		// Set mIsSurfaceReady to 'true' *before* making a call to handleResume
		NativePlayer.mIsSurfaceReady = true;
		NativePlayer.onNativeSurfaceChanged();
			...
			new Thread(new Runnable() {
				...
				public void run() {
					try {
						...
						if (!NativePlayer.mExitCalledFromJava) {
							NativePlayer.handleNativeExit();
						}
					}
				}
			}).start();
	...
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		...
		(event.getSource() & InputDevice.SOURCE_DPAD) != 0) {
			if (event.getAction() == KeyEvent.ACTION_DOWN) {
				if (NativePlayer.onNativePadDown(event.getDeviceId(), keyCode) == 0) {
					return true;
				}
			} else if (event.getAction() == KeyEvent.ACTION_UP) {
				if (NativePlayer.onNativePadUp(event.getDeviceId(), keyCode) == 0) {
					return true;
				}
			}
		}
		...
		if ((event.getSource() & InputDevice.SOURCE_KEYBOARD) != 0) {
			if (event.getAction() == KeyEvent.ACTION_DOWN) {
				...
				NativePlayer.onNativeKeyDown(keyCode);
				return true;
			} else if (event.getAction() == KeyEvent.ACTION_UP) {
				// Log.v("SDL", "key up: " + keyCode);
				NativePlayer.onNativeKeyUp(keyCode);
				return true;
			}
		}
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_MOVE:
			for (i = 0; i < pointerCount; i++) {
				...
				NativePlayer.onNativeTouch(touchDevId, pointerFingerId, action,
						x, y, p);
			}
			break;
		...
		case MotionEvent.ACTION_POINTER_DOWN:
			...
			y = event.getY(i) / mHeight;
			p = event.getPressure(i);
			NativePlayer.onNativeTouch(touchDevId, pointerFingerId, action, x,
					y, p);
			break;
		...
		case MotionEvent.ACTION_CANCEL:
			for (i = 0; i < pointerCount; i++) {
				...
				y = event.getY(i) / mHeight;
				p = event.getPressure(i);
				NativePlayer.onNativeTouch(touchDevId, pointerFingerId,
	...
	public void onSensorChanged(SensorEvent event) {
		if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
			...
			NativePlayer.onNativeAccel(-x / SensorManager.GRAVITY_EARTH, y
...
class DummyEdit extends View implements View.OnKeyListener {
	...
	public boolean onKeyPreIme(int keyCode, KeyEvent event) {
		...
		if (event.getAction() == KeyEvent.ACTION_UP
			...
			if (NativePlayer.mTextEdit != null
					&& NativePlayer.mTextEdit.getVisibility() == View.VISIBLE) {
				NativePlayer.onNativeKeyboardFocusLost();
			}
		}
...
class SDLJoystickHandler_API12 extends SDLJoystickHandler {
	...
	public void pollInputDevices() {
		...
		for (int i = deviceIds.length - 1; i > -1; i--) {
			...
			if (joystick == null) {
				...
				if ((joystickDevice.getSources() & InputDevice.SOURCE_CLASS_JOYSTICK) != 0) {
					...
					NativePlayer.nativeAddJoystick(joystick.device_id,
		...
		for (int i = 0; i < removedDevices.size(); i++) {
			...
			NativePlayer.nativeRemoveJoystick(device_id);
			...
			case MotionEvent.ACTION_MOVE:
				...
				if (joystick != null) {
					for (int i = 0; i < joystick.axes.size(); i++) {
						...
						NativePlayer.onNativeJoy(joystick.device_id, i, value);
					}
					for (int i = 0; i < joystick.hats.size(); i += 2) {
						...
						NativePlayer.onNativeHat(joystick.device_id, i / 2,
								hatX, hatY);
					}
				}
				break;
...
class SDLGenericMotionListener_API12 implements View.OnGenericMotionListener {
	...
	public boolean onGenericMotion(View v, MotionEvent event) {
		return NativePlayer.handleJoystickMotionEvent(event);
	}
}
